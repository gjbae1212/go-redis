...
func (c *Group) addPool(addr string) error {
	if addr == "" {
		return fmt.Errorf("[err] addPool %w", internal.ErrorRedisEmpty)
	}
	c.Lock()
	defer c.Unlock()

	if _, ok := c.Pools[addr]; ok {
		return fmt.Errorf("[err] addPool %w", internal.ErrorRedisAlready)
	}
...
func (c *Group) deletePool(addr string) error {
	if addr == "" {
		return fmt.Errorf("[err] deletePool %w", internal.ErrorRedisEmpty)
	}
	c.Lock()
	defer c.Unlock()
	...
	if !ok {
		return fmt.Errorf("[err] deletePool %w", internal.ErrorRedisNotFound)
	}
...
func (c *Group) getPoolByKey(key string) (*Pool, error) {
	if key == "" {
		return nil, fmt.Errorf("[err] getPoolByKey %w", internal.ErrorRedisEmpty)
	}
...
func (c *Group) getPoolByAddr(addr string) (*Pool, error) {
	if addr == "" {
		return nil, fmt.Errorf("[err] getPoolByAddr %w", internal.ErrorRedisEmpty)
	}
	...
	return nil, fmt.Errorf("[err] getPoolByAddr %w", internal.ErrorRedisNotFound)
}

func (c *Group) do(timeout time.Duration, command string, args ...interface{}) (interface{}, error) {
	if command == "" || len(args) < 1 {
		return nil, fmt.Errorf("[err] do %w", internal.ErrorRedisEmpty)
	}
...
func (c *Group) multi(timeout time.Duration, command string, args ...interface{}) ([]interface{}, error) {
	if command == "" || len(args) == 0 {
		return nil, fmt.Errorf("[err] multi %w", internal.ErrorRedisEmpty)
	}

	if !strings.HasPrefix(command, "MSET") && !strings.HasPrefix(command, "MGET") {
		return nil, fmt.Errorf("[err] multi %w", internal.ErrorRedisInvalid)
	}
...
func keyString(key interface{}) (string, error) {
	...
	case string:
		return key, nil
	default:
		return "", fmt.Errorf("[err] keystring %w", internal.ErrorRedisInvalid)
	}
}
